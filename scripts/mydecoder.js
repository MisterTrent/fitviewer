import Accumulator from "../fitscripts/src/accumulator.js";
import BitStream from "../fitscripts/src/bit-stream.js";
import CrcCalculator from "../fitscripts/src/crc-calculator.js";
import FIT from "../fitscripts/src/fit.js";
import HrMesgUtils from "../fitscripts/src/utils-hr-mesg.js";
import Profile from "../fitscripts/src/profile.js";
import Stream from "../fitscripts/src/stream.js";
import Utils from "../fitscripts/src/utils.js";
import UtilsInternal from "../fitscripts/src/utils-internal.js";
...
class MyDecoder {
    ...
    //storing error position only for now. First one stop the read process,
    //but may later modify read process to skip error records *if possible*
    errorBytes = [];
    
    byteArrayDispatch = undefined;
    ...
    constructor(stream) {
        ...
        //adding storage for the messages as they're read from the file
        this.byteArrayDispatch ={'data': this.messageBytes,
                                 'definition': this.definitionBytes,
                                 'unknown' : this.messageBytes,
                                 'compressedTimestamp': this.compressedBytes,
                                 'error' : this.errorBytes};
    ...
    static isFIT(stream) {
        try {
            ...
            const fileHeader = MyDecoder.#readFileHeader(stream, true);
     ...
     */
    isFIT() {
        return MyDecoder.isFIT(this.#stream);
    }
    ...
    checkIntegrity() {
        try {
            ...
            const fileHeader = MyDecoder.#readFileHeader(this.#stream, true);
    ...
    #decodeNextFile() {
        ...
        const fileHeader = MyDecoder.#readFileHeader(this.#stream);
        
        let obj = {'startByte' : position,
                   'endByte' : this.#stream.position - 1,
                   'length' : this.#stream.position - position,
                   'type': 'fileheader',
                   'subtype' : null,
                   'data' : fileHeader};

        this.allBytes.push(obj)
    ...
    #decodeNextRecord() {
        ...
        let localMesgNum = recordHeader & LOCAL_MESG_NUM_MASK;
        let globalMesgNum = null;

        let position = this.#stream.position;
        let output;
        let outputType;
        let outputSubtype;
        ...
        try {
            if ((recordHeader & MESG_DEFINITION_MASK) === MESG_HEADER_MASK) {
                let def = this.localMessageDefinitions[localMesgNum];
                output = this.#decodeMessage();

                if (output == undefined) {
                    //if including unknown data, this block won't be reached
                    output = null; 
                    outputType = 'unknown';
                    outputSubtype = null;
                    ...
                    this.#messages[def.messagesKey].push(output);
                    outputType = 'data';
                    outputSubtype = def.name ? def.name : null;               
                }
            ...
            if ((recordHeader & COMPRESSED_HEADER_MASK) === COMPRESSED_HEADER_MASK) {
                //current SDK hardcodes an error for this
                output = this.#decodeCompressedTimestampDataMessage();
                outputType = 'compressedTimestamp';
            }
            
            if ((recordHeader & MESG_DEFINITION_MASK) === MESG_DEFINITION_MASK) {
                output = this.#decodeMessageDefinition();
                globalMesgNum = output.globalMessageNumber;
                
                this.localMessageDefinitions[localMesgNum] = output;
                outputType = 'definition';
                outputSubtype = output.name ? output.name : null;
            }

        } catch (e) {
            //SDK currently stops read at first error 
            ...
            let obj = {'startByte' : position,
                       ...
                       'has_profile' : Profile.messages.hasOwnProperty(globalMesgNum),
                       'data' : output,
                       'localMesgNumber' : localMesgNum ?? null,
                       'globalMesgNumber' : globalMesgNum,
                       'messageNumber' : this.allBytes.length};

            this.allBytes.push(obj)
            this.byteArrayDispatch[outputType].push(obj);
        }
            
    }

    /**
     *overriding SDK method to alter this to return definition
     */
    #decodeMessageDefinition() {
        ...
        //my change, making this a return
        ...
        return { ...messageDefinition, ...messageProfile };
    }    
    ...
    #decodeMessage() {
        ...
        //renamed 'fields' to fieldProfiles to reflect data origin (SDK Profiles)
        const fieldProfiles = messageDefinition.fields ?? {};
        ...
        messageDefinition.fieldDefinitions.forEach(fieldDefinition => {
            //undefined if definition globalmesgnum wasn't in SDK Profiles
            //as fieldProfiles will be empty
            const field = fieldProfiles[fieldDefinition.fieldDefinitionNumber];       
            const { fieldName, rawFieldValue } = this.#readFieldValue(messageDefinition, fieldDefinition, field);
            
            //there'll always be a fieldName if there's a messagedefinition,
            //as the fieldnumber from there is used as backup
            if (fieldName != null && (field != null || this.#optIncludeUnknownData)) {
                ...
                //skip ahead if field has no profile to avoid throwing error
                if (field !== undefined){
        ...
        if (mesgNum === Profile.MesgNum.DEVELOPER_DATA_ID) {
            ...
            const key = Object.keys(this.#developerDataDefinitions)
                .reduce((count, key) => count + this.#developerDataDefinitions[key].fieldProfiles.length, 0);
            ...
            this.#expandComponents(mesgNum, message, fieldProfiles);
            this.#transformValues(message, messageDefinition);
        }

        //TODO why is 'name' the field to check? Does that serve as ruling out non-SDK-profile definitions?
